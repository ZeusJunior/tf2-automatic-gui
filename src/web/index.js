...
const app = new Vue({
	...
	methods: {
		...
		saveItem: async function(item, edit, fromModal) {
			if (item.name === '') {
				app.$refs.msg.sendMessage('danger', 'Item name is missing');
				if (fromModal) {
			...
			app.$refs.msg.sendMessage(res.data.msg.type, res.data.msg.message);
			if (fromModal ) {
		...
		deleteItem: function(item) {
			...
			$.ajax({
				...
				success: function(data, status) {
					if (data.removed > 0) {
						app.$refs.msg.sendMessage('success', 'successfully removed ' + data.removed + (data.removed === 1 ? ' item':' items'));
					}
					app.loadItems();
				},
				dataType: 'json',
		...
		bulkAdd: function(bulk) {
			...
			app.$refs.msg.sendMessage('secondary', 'Bulk add started, it might take a while.');
			...
			axios({
				...
				.then((resp)=>{
					app.$refs.msg.sendMessage(resp.data.msg.type, resp.data.msg.message);
					app.loadItems();
				});
		},
			...
			if (this.multiSelect.all) {
				$.ajax({
					...
					success: function(data, status) {
						...
						app.$refs.msg.sendMessage(data.msg.type, data.msg.message);
						app.loadItems();
					},
					dataType: 'json',
					traditional: true
